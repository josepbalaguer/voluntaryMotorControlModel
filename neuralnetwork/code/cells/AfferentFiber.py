...
class AfferentFiber(Cell):
	...
	def update(self,time,specialSimulation):
		self._update_activity(time,specialSimulation)
		if self._record: self._record_segment(time)

	def _update_activity(self,time,specialSimulation):
		...
		specialSimulation -- #boolean variable to simulate spatiotemporal SCS
		"""


		if specialSimulation:
		# Spatiotemporal stimulation (SCS activated initstim (500 ms) after CST activation
			initstim = 500
			if h.t < 4000+initstim or (h.t > 6000 and h.t < 8000+initstim) or (
					h.t > 10000 and h.t < 12000+initstim): self.cell.EES = 0
