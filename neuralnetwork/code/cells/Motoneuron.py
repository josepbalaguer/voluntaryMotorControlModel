...
class Motoneuron(Cell):
	...
	def __init__(self,drug=True):
		...
		self._drug = drug
		self.synapses = []
		self.dendritesegment_Iaf=[]
		self.dendritenumber_Iaf=[]
		self.dendritesegment_CM=[]
		self.dendritenumber_CM=[]
	...
	def create_synapse(self, type, source):
		...
		if type=="excitatory":
			...
			x = rnd.random()
			if source =="Iaf":
				self.dendritenumber_Iaf.append(n)
				self.dendritesegment_Iaf.append(x)
			elif source =="CM":
				self.dendritenumber_CM.append(n)
				self.dendritesegment_CM.append(x)
	...
	def get_dendrite_Iaf(self):
		return [self.dendritesegment_Iaf, self.dendritenumber_Iaf]

	def get_dendrite_CM(self):
		return [self.dendritesegment_CM, self.dendritenumber_CM]
