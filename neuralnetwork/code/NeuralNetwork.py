...
class NeuralNetwork():
	...
	def __init__(self, parallelContext, inputFile):
		...
		self._corticoMotoneuronsNames = []
		self._weightdebug=[]
		self._weightdebug_Iaf = []
		self._weightdebug_CST = []
		self._delaydebug=[]
		...
		for muscle,muscAfferentDelay in self._infoMuscles:
			...
			for cellInfo in self._infoCommonCellsInMuscles:
				...
				if (cellClass=="Motoneuron" or cellClass=="IntFireMn") and self.recordMotoneurons:
					...
					self.actionPotentials[muscle][cellName]=[]
				elif cellClass=="CSNeuron" and self.recordAfferents:
					self.actionPotentials[muscle][cellName]=[]
	...
	def _create_cells(self):
		...
		for muscle,muscAfferentDelay in self._infoMuscles:
			for cellInfo in self._infoCommonCellsInMuscles:
				...
				cellName = cellInfo[1]
				if cellInfo[2] == 'def':
					if muscle == 'TRI': cellNumber=193
					elif muscle == 'BIC': cellNumber = 119
				else: cellNumber = cellInfo[2]
		...
		self._afferentsNames = self._primaryAfferentsNames+self._secondaryAfferentsNames + self._corticoMotoneuronsNames

		#Iterate through cellsId to obtain Iaf CellName
		self._key.append(list(self.cellsId.keys())[0])
		for CellinMuscle in range(len(self._infoCommonCellsInMuscles)):
			self._key.append(list(self.cellsId[self._key[0]].keys())[CellinMuscle])  # key = ['muscle','Iaf']
		...
		for n in range(int(cellNumber)):
			...
			if n%sizeComm==rank:
				...
				if cellClass=="IntFireMn":
					...
					self._drug=False
					if neuronParam=="drug":self._drug=True
					self.cells[muscle][cellName].append(Motoneuron(self._drug))
				...
				elif cellClass=="CSNeuron":
					#List containing all supraspinal names
					if not cellName in self._corticoMotoneuronsNames: self._corticoMotoneuronsNames.append(cellName)
					delay = 7 #ms Williams and baker 2009
					# fineTask - parameter specific to the CM
					self._fineTask = False
					if neuronParam == "fineTask": self._fineTask = True
					self.cells[muscle][cellName].append(CSFiber(delay,self._fineTask))
				else:
	...
	def _connect(self,sourcesId,targetsId,conRatio,conNum,conWeight,synType, conDelay=1):
		...
		noisePerc = 0.2
		noisePerc_delay = 0.2
		noisePerc_weight = 0.2
		sourceCellName = "excitatoryfiber"
		for targetId in targetsId:
			...
			for i in range(conNum):
				...
				if synType == "excitatory" or synType == "inhibitory":
					if 'Iaf' in self.get_primary_afferents_names():
						if source in self.cellsId[self._key[0]][self._key[self._key.index('Iaf')]]: sourceCellName = "Iaf"
					if 'CM' in self.get_corticomotoneurons_names():
						if source in self.cellsId[self._key[0]][self._key[self._key.index('CM')]]: sourceCellName = "CM"
					target = cell.create_synapse(synType, sourceCellName)
				nc = self._pc.gid_connect(source,target)
				if sourceCellName=="Iaf":
					nc.weight[0] = rnd.normalvariate(conWeight, conWeight * noisePerc)
					self._weightdebug_Iaf.append(nc.weight[0])
				elif sourceCellName=="CM":
					nc.weight[0] = rnd.normalvariate(conWeight, conWeight * noisePerc_weight)
					self._weightdebug_CST.append(nc.weight[0])
				if nc.weight[0]<0: nc.weight[0]=0
				self._weightdebug.append(nc.weight[0])
				if sourceCellName=="Iaf":
					nc.delay = rand.lognormal(-0.47, 0.37) #Greiner 2021
				elif sourceCellName=="CM":
					nc.delay = conDelay + rnd.normalvariate(0.25, 0.25 * noisePerc_delay)
				else:
					nc.delay = conDelay+rnd.normalvariate(0.25,0.25*noisePerc)
				if nc.delay < 0: nc.delay = 0
				self._delaydebug.append(nc.delay)
	...
	def _create_common_connections(self):
		...
		for muscle,muscAfferentDelay in self._infoMuscles:
			for connection in self._infoCommonMuscleConnections:
				...
				if connection[4]=='def':
					tau = 0.2 # synapse time constant (in ms) - Greiner 2021
					if muscle == 'TRI': gmax = 3.375 * 1e-3  # synapse maximal conductance (in pS * 1e-3) - Greiner 2021
					elif muscle == 'BIC': gmax = 5 * 1e-3  # synapse maximal conductance (in pS * 1e-3) - Greiner 2021
					conWeight = gmax/tau
				else: conWeight = float(connection[4])
				# Type of synapse
			...
			for M2weights,M1 in zip(matrix[1:],self._infoMuscles):
				for weight,M2 in zip(M2weights[1:],self._infoMuscles):
					if not float(weight) == 0:
						if M1[0] is M2[0]: raise Exception("Intra muscle sensorimotor connections have to be implemented in section 4.")
	...
	def update_afferents_ap(self,time):
		...
		specialSimulation = False #boolean variable to simulate spatiotemporal SCS
		# Iterate over all dictionaries
		affnum=0
			...
			for cellName in self.cells[muscle]:
				if cellName in self._afferentsNames:
					for cell in self.cells[muscle][cellName]:
						if cellName in self.get_corticomotoneurons_names(): cell.update(time)
						elif cellName in self.get_primary_afferents_names():
							affnum+=1
							cell.update(time,specialSimulation)
	...
	def set_afferents_fr(self,fr):
		...
		for muscle in self.cells:
			for cellName in self.cells[muscle]:
				if cellName in self._primaryAfferentsNames:
					for cell in self.cells[muscle][cellName]:
	...
	def get_corticomotoneurons_names(self):
		""" Return the inteurons names. """
		return self._corticoMotoneuronsNames
	...
	def get_weight_info(self):
		""" Return the connection informations. """
		return self._weightdebug

	def get_delay_info(self):
		""" Return the connection informations. """
		return self._delaydebug

	def get_weight_Iaf_info(self):
		""" Return the connection informations. """
		return self._weightdebug_Iaf

	def get_weight_CST_info(self):
		""" Return the connection informations. """
		return self._weightdebug_CST