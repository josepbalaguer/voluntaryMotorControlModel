...
class CST():
	...
	def __init__(self,parallelContext,neuralNetwork,amplitude,frequency,timestart,pulsesNumber=100000,species="rat"):
		...
		self._eesId = 1000005 + timestart +5
		...
		for muscle in self._nn.cellsId:
			...
			for cellType in self._nn.cellsId[muscle]:
				if cellType in self._recruitedCells:
					...
					if cellType in self._nn.get_corticomotoneurons_names():
	...
	def _activate_connections(self,netcons,percentage):
		...
		ncIndexes=self._idxstim
		netcons[ncIndexes].active(True)
	...
	def set_amplitude(self,amplitude,muscles=None):
		...
		Keyword arguments:
		amplitude -- Amplitude of stimulation. It could either be an integer
		value between _minCur and _maxCur or a list containing the percentages
		of recruited primary afferents, secondary afferents and motoneurons.
	...
	def set_frequency(self,frequency):
		...
		"""
		if rank == 0:
			if frequency>2 and frequency<self._maxFrequency:
				self._frequency = frequency
				self._stim.interval = 1000.0/self._frequency
			elif frequency<=2:
				self._frequency = 0
				self._stim.interval = 20000
			...
			self._freqdebug = self._stim.interval
			self._freqdebug2 = frequency
